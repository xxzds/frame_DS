...
/**
 * 自定义session，写法仿照org.apache.shiro.session.mgt.SimpleSession
 * 扩展session中的字段userId、userName、userAgent、status、systemHost，便于在线用户的展示
 * @author ding.shuai
 * @date 2016年9月12日下午5:04:04
 */
public class MySession extends SimpleSession {
    ...
    private transient static final Logger log = LoggerFactory.getLogger(MySession.class);
    ...
    /**
     * Returns {@code true} if the specified argument is an {@code instanceof} {@code CustomSession} and both
     * {@link #getId() id}s are equal.  If the argument is a {@code CustomSession} and either 'this' or the argument
     * does not yet have an ID assigned, the value of {@link #onEquals(MySession) onEquals} is returned, which
     * does a necessary attribute-based comparison when IDs are not available.
     * <p/>
     * Do your best to ensure {@code CustomSession} instances receive an ID very early in their lifecycle to
     ...
     *
     * @param ss the CustomSession instance to compare for equality.
    ...
    /**
     * Returns the string representation of this CustomSession, equal to
     * <code>getClass().getName() + &quot;,id=&quot; + getId()</code>.
     *
     * @return the string representation of this CustomSession, equal to
     *         <code>getClass().getName() + &quot;,id=&quot; + getId()</code>.
     * @since 1.0
     */
    @Override
