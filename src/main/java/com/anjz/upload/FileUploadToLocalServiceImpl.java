 ...
 * 上传到本地 改版，增加上传限制
 * @author ding.shuai
 * @date 2016年9月21日下午4:16:42
 */
public class FileUploadToLocalServiceImpl implements FileUploadService {
	
	private static final Logger logger=LoggerFactory.getLogger(FileUploadToLocalServiceImpl.class);

	// 默认大小 50M
	private long DEFAULT_MAX_SIZE = 52428800;

	// 默认上传的地址
	private String defaultBaseDir = "upload";
	...
	private String[] FLASH_EXTENSION = {
            "swf", "flv"
    };
	...
	@Override
	public PlainResult<String> uploadFile(MultipartFile file,String baseDir,String[] allowedExtension,long maxSize,boolean needDatePathAndRandomName ) {
		...
		try {
        	...
        	LogUtils.logError("upload.not.allow.image.extension", e);  
        	...
        	LogUtils.logError("upload.not.allow.flash.extension", e);    
        	...
        	LogUtils.logError("upload.not.allow.media.extension", e);    
	...
	public void delete(String filename) throws IOException {
        ...
        File desc = getAbsoluteFile(extractUploadDir(), filename);
        if (desc.exists()) {
            desc.delete();
            logger.info("{}用户[{}]删除了上传的文件{}", DateUtil.formatDate(new Date()),SecurityUtils.getSubject().getPrincipal(),filename);
        }
    }
    ...
    private String upload(
            String baseDir, MultipartFile file,
     ...
     * @throws InvalidExtensionException  MIME类型不允许
     ...
     * @param file  上传的文件
     * @param baseDir 基路径
     * @param needDatePathAndRandomName  是否需要加上时间路径
     * @return
     ...
     * @return
     */
    private String extractUploadDir() {
