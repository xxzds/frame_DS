...
<div class="panel">
    ...
    <ul class="nav nav-tabs">
        ...
        <c:if test="${op eq '新增'}">
            <c:forEach items="${types}" var="t">
                <li ${type eq t ? 'class="active"' : ''}>
                    <a href="${ctx}/maintain/icon/${t}/create?BackURL=<es:BackURL/>">
        ...
        <c:if test="${not empty m.id}">
        	<shiro:hasPermission name="maintain:icon:views">
            <li ${op eq '查看' ? 'class="active"' : ''}>
                <a href="${ctx}/maintain/icon/${m.id}?BackURL=<es:BackURL/>">
            ...
            <li ${op eq '修改' ? 'class="active"' : ''}>
                <a href="${ctx}/maintain/icon/${m.id}/update?BackURL=<es:BackURL/>">
                    <i class="icon-edit"></i>
                    修改
                </a>
            </li>
            ...
            <li ${op eq '删除' ? 'class="active"' : ''}>
                <a href="${ctx}/maintain/icon/${m.id}/delete?BackURL=<es:BackURL/>">
    ...
    <form:form id="editForm" method="post" commandName="m" cssClass="form-horizontal" enctype="multipart/form-data">
            ...
            <c:if test="${type eq 'css_sprite'}">
                ...
                <div class="control-group">
                    ...
                    <div class="controls">
                        <form:input path="spriteSrc"/> <span class="muted font-12">绝对路径：如http://www.abc.com，相对路径不要加上下文</span>
                    </div>
                </div>
                <div class="control-group">
                    <form:label path="offsetLeft" cssClass="control-label">位置</form:label>
                    <div class="controls">
                        <form:input path="offsetLeft" cssClass="validate[required, custom[integer]] input-small"
                                    placeholder="距离左边" data-toggle="tooltip" data-placement="bottom" title="距离左边"/>
                        <form:input path="offsetTop" cssClass="validate[required, custom[integer]] input-small"
        ...
        <c:choose>
            ...
            <c:otherwise>
                ...
                $.validationEngineLanguage.allRules.ajaxIdentityCall= {
                    "url": "${ctx}/maintain/icon/validate",
                    //动态提取的数据。验证时一起发送
                    extraDataDynamic : ['#id'],
                    //验证失败时的消息
