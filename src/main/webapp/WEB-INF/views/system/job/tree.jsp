...
<ul class="nav nav-tabs">
    <li ${empty param['search.is_show|eq'] ? 'class="active"' : ''}>
        <a href="${ctx}/system/job/tree?async=${not empty param.async and param.async eq true}">
    ...
    <li ${not empty param['search.is_show|eq'] ? 'class="active"' : ''}>
        <a href="${ctx}/system/job/tree?search.is_show|eq=1&async=${not empty param.async and param.async eq true}">
            <i class="icon-table"></i>
            显示的
        </a>
    </li>
...
<script type="text/javascript">
    ...
    $(function() {
        ...
        $.zTree.initMovableTree({
            ...
            permission: <es:treePermission resourceIdentity="system:job"/>,
            autocomplete : {
                enable : true
            },
            ...
            setting : {
                callback : {
                    onClick: function(event, treeId, treeNode, clickFlag) {
                        parent.frames['listFrame'].location.href='${ctx}/system/job/' + treeNode.id + "/update?async=" + async ;
                    }
                }
            }
        });
