...
<table id="table" class="sort-table table table-bordered table-hover" data-async="true">
    ...
    <tr>
        ...
        <c:if test="${type eq 'user'}">
            <th style="width: 150px" sort="user_id">用户名</th>
        </c:if>
        <c:if test="${type eq 'user_group'}">
            <th style="width: 150px" sort="group_id">用户组名称</th>
        </c:if>
        <c:if test="${type eq 'organization_job'}">
            <th style="width: 150px" sort="organization_id">组织机构</th>
            <th style="width: 150px" sort="job_id">工作职务</th>
        </c:if>
        <c:if test="${type eq 'organization_group'}">
            <th style="width: 150px" sort="group_id">组织机构组</th>
        </c:if>
    ...
    <c:forEach items="${page.content}" var="m">
        <tr>
            ...
            <td>
                <a class="btn btn-link btn-edit" href="${ctx}/system/permission/auth/${m.id}">${m.id}</a>
            </td>
            <c:if test="${type eq 'user'}">
            ...
            <td>
                ...
                <c:forEach items="${m.roleIdsArray}" var="roleId">
