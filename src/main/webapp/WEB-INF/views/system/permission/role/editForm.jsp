...
<div class="panel">
    ...
    <ul class="nav nav-tabs">
        ...
        <c:if test="${op eq '新增'}">
            <li ${op eq '新增' ? 'class="active"' : ''}>
                <a href="${ctx}/system/permission/role/create?BackURL=<es:BackURL/>">
        ...
        <c:if test="${not empty m.id}">
            <li ${op eq '查看' ? 'class="active"' : ''}>
                <a href="${ctx}/system/permission/role/${m.id}?BackURL=<es:BackURL/>">
            ...
            <li ${op eq '修改' ? 'class="active"' : ''}>
                <a href="${ctx}/system/permission/role/${m.id}/update?BackURL=<es:BackURL/>">
                    <i class="icon-edit"></i>
                    修改
                </a>
            </li>
            ...
            <li ${op eq '删除' ? 'class="active"' : ''}>
                <a href="${ctx}/system/permission/role/${m.id}/delete?BackURL=<es:BackURL/>">
    ...
    <form:form id="editForm" method="post" commandName="m" cssClass="form-inline form-horizontal form-small">
        ...
        <div id="roleInfo">
            ...
            <div class="control-group span4">
                <form:label path="isShow" cssClass="control-label">状态</form:label>
                <div class="controls inline-radio">
                    <form:radiobuttons
                            path="isShow" items="${availableList}" itemLabel="info" itemValue="value" cssClass="validate[required]"/>
                </div>
            </div>
        ...
        <div id="resourcePermissionInfo" class="span10">
            ...
            <div id="selectedResourcePermssion" class="span9">
                ...
                <table class="table table-bordered table-hover">
                    ...
                    <tbody>
                    <c:forEach items="${resouceIdAndPermissionIds}" var="o">
                    <c:if test="${esfn:existsResource(o.key, onlyDisplayShow)}">
                    <tr>
                        <td class="check"><input type="checkbox"></td>
                        <td>
                            <input type='hidden' id='resourceId_${o.key}' name='resourceId' value='${o.key}'>
                            <sys:showResourceName id="${o.key}"/>
                        </td>
                        <td>
                            <span style="line-height: 30px">
                            <c:set var="permissionIds" value=""/>
                            <c:set var="count" value="0"/>
                            <c:forEach items="${o.value}" var="permissionId">
...
<script type="text/javascript">
    $(function () {
        <c:choose>
            ...
            <c:otherwise>
                ...
                $.validationEngineLanguage.allRules.ajaxNameCall= {
                    "url": "${ctx}/system/permission/role/validate",
                    //动态提取的数据。验证时一起发送
                    extraDataDynamic : ['#id'],
                    //验证失败时的消息
        ...
        $(".btn-add-resource-permission").click(function() {
            ...
            var permissionNames = $selectedPermissions.map(function() {return $(this).html();}).get().join(",");
