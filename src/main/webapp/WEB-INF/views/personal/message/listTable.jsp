...
<table id="table" class="sort-table table table-bordered table-hover" data-async="true">
    ...
    <c:forEach items="${page.content}" var="m">
        <tr>
            ...
            <%--<td>--%>
                <%--<a class="btn btn-link btn-edit" href="${ctx}/personal/message/${m.id}">${m.id}</a>--%>
            <%--</td>--%>
            <td>
                <c:choose>
                    <c:when test="${state eq 'draft_box'}">
                        <a href="${ctx}/personal/message/draft/${m.id}/send" class="btn btn-link no-padding">${m.title}</a>
                    </c:when>
                    <c:otherwise>
                        <a href="${ctx}/personal/message/${m.id}" class="btn btn-link no-padding">${m.title}</a>
                        <c:if test="${m.isRead eq false}">
            ...
            <td><fmt:formatDate value="${m.sendDate}" pattern="yyyy-MM-dd hh:mm:ss" type="date" dateStyle="long" />
			</td>
        </tr>
    </c:forEach>
    </tbody>
