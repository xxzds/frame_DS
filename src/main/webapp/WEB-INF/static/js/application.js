    ...
    initCommonBtn : function() {
        $(".btn-view-info,.btn-change-password").click(function() {
            ...
            if(a.is(".btn-view-info")) {
                url = ctx +"/system/user/loginUser/viewInfo";
            } else if(a.is(".btn-change-password")) {
                url = ctx + "/system/user/loginUser/changePassword";
            }
            setTimeout(function() {
                $.tabs.activeTab(100000, "个人资料", url, true)
            }, 0);
        });
        $(".btn-view-message,.btn-message").click(function() {
            var url = ctx + "/personal/message/in_box/list";
            if($(this).is('.btn-view-message')){
            	 setTimeout(function() {
                     $.tabs.activeTab(100001, "我的消息", url, true)
                 }, 0);
            }
        });
...
$.movable = {
    ...
    initMoveableTable : function(table) {
        ...
        if(!urlPrefix) {
            $.app.alert({message : "请添加移动地址URL，如&lt;table data-move-url-prefix='/sample'&gt;<br/>自动生成：/sample/{fromId}/{toId}/{direction:方向(up|down)}"});
        }
